...
export interface LongpathContent {
  ...
  download: {
    ...
    kanalKopf: string;
    fuerDieses: string; stand: string; standUngeprueft: string; standWindows: string; herunterladen: string; alleAlternativ: string;
    herkunftHtml: string;
  };
}
  ...
  de: {
    ...
    werkzeuge: [
      ...
      { bereich: "Logbuch", titel: "Verbindungen, die sich wiederfinden lassen",
        text: "Aus der schmalen Leiste wird ein voller Arbeitsplatz: suchen über Rufzeichen, Name, QTH, Land, Feld oder Bemerkung, filtern nach Band und Betriebsart, ADIF und Cabrillo hinein und hinaus. Daneben die Stammdaten von QRZ und die Peilung dorthin. „Sync QRZ“ holt das eigene QRZ-Logbuch ab und führt es wie einen Import zusammen. Hinter „Stats…“ sechs Kacheln: Bänder, Betriebsarten, Aktivität, Länder, dazu DXCC, WAS, Grids und Zonen.",
        marken: ["ADIF", "Cabrillo", "Sync QRZ", "DXCC / WAS"] },
      { bereich: "Karte", titel: "Die Karte fliegt zur Station",
        text: "Rufzeichen eingeben, Enter — die Karte fliegt dorthin, wo QRZ die Station verortet, zeichnet den Großkreis vom eigenen Standort und nennt Entfernung und Peilung. Darunter Luftbild von NASA GIBS, ohne Schlüssel. Amateurfunksatelliten über dem Horizont stehen als Dreiecke auf Kugel und Karte, und jedes geloggte QSO bekommt mitgeschrieben, welche gerade in Sicht waren.",
        marken: ["Großkreis", "NASA GIBS", "Satelliten"] },
      ...
      { bereich: "Digital", titel: "RTTY direkt aus dem Empfangston",
      ...
      { bereich: "Digital", titel: "Morse direkt aus dem Empfangston", neu: true,
        text: "Ein eigener CW-Decoder, ebenfalls ohne fldigi. Tonhöhe und Tempo erkennt er selbst, beides lässt sich festhalten; das Suchband folgt dem eingestellten CW-Pitch. Unsichere Zeichen erscheinen gedämpft, Rauschen wird gar nicht erst angezeigt.",
        marken: ["CWL / CWU", "LOCK Hz / WPM"] },
      { bereich: "Speicher", titel: "Frequenzen und Empfang unter einem Namen", neu: true,
        text: "Frequenzspeicher im Format von Thetis — eine vorhandene memory.xml lässt sich einfach hineinkopieren — mit Gruppe, Name, Modus, Filter, AGC und Repeater-Ablage je Platz. Getrennt davon RX-Profile: AGC, Rauschminderung, Notch, Störaustaster und Rauschsperre einer Scheibe, mit einem Klick gespeichert und wieder geladen.",
        marken: ["memory.xml", "Quick Save", "RX-Profile"] },
    ],
    ...
    download: {
      ...
      standUngeprueft: "Signiert · ungeprüft",
      standWindows: "Signiert · ungeprüft · Tonproblem offen",
  ...
  en: {
    ...
    werkzeuge: [
      ...
      { bereich: "Logbook", titel: "Contacts you can actually find again",
        text: "The narrow strip becomes a full workspace: search by callsign, name, QTH, country, grid or remark, filter by band and mode, ADIF and Cabrillo in and out. Alongside it, QRZ master data and the bearing there. \"Sync QRZ\" fetches your own QRZ logbook and merges it like an import. Behind \"Stats…\", six tiles: bands, modes, activity, countries, plus DXCC, WAS, grids and zones.",
        marken: ["ADIF", "Cabrillo", "Sync QRZ", "DXCC / WAS"] },
      { bereich: "Map", titel: "The map flies to the station",
        text: "Type a callsign, press Enter — the map flies to where QRZ places the station, draws the great circle from your own location and gives distance and bearing. Underneath, NASA GIBS imagery, no key needed. Amateur satellites above the horizon appear as triangles on globe and map, and every logged QSO records which ones were in view at the time.",
        marken: ["Great circle", "NASA GIBS", "Satellites"] },
      ...
      { bereich: "Foreign radios", titel: "Even what doesn't speak OpenHPSDR",
        ...
        marken: ["SunSDR2 QRP", "KiwiSDR"] },
      { bereich: "Digital", titel: "RTTY straight from the receive audio",
      ...
      { bereich: "Digital", titel: "Morse straight from the receive audio", neu: true,
        text: "A CW decoder of its own, again without fldigi. It finds pitch and speed by itself, and both can be locked; the search band follows the CW pitch setting. Uncertain characters appear dimmed, noise is not shown at all.",
        marken: ["CWL / CWU", "LOCK Hz / WPM"] },
      { bereich: "Memories", titel: "Frequencies and receive settings under a name", neu: true,
        text: "Frequency memories in Thetis' format — an existing memory.xml can simply be copied in — with group, name, mode, filter, AGC and repeater offset per slot. Separately, RX profiles: a slice's AGC, noise reduction, notch, noise blanker and squelch, saved and recalled with one click.",
        marken: ["memory.xml", "Quick Save", "RX profiles"] },
    ],
    ...
    download: {
      ...
      standUngeprueft: "Signed · unverified",
      standWindows: "Signed · unverified · audio issue open",
  ...
  es: {
    ...
    werkzeuge: [
      ...
      { bereich: "Cuaderno de guardia", titel: "Contactos que de verdad se vuelven a encontrar",
        text: "La franja estrecha se convierte en un puesto de trabajo completo: buscar por indicativo, nombre, QTH, país, locator o comentario, filtrar por banda y modo, ADIF y Cabrillo de entrada y salida. Junto a eso, los datos de QRZ y el rumbo hasta allí. «Sync QRZ» descarga el propio cuaderno de QRZ y lo fusiona como una importación. Tras «Stats…», seis fichas: bandas, modos, actividad, países, además DXCC, WAS, locators y zonas.",
        marken: ["ADIF", "Cabrillo", "Sync QRZ", "DXCC / WAS"] },
      { bereich: "Mapa", titel: "El mapa vuela hasta la estación",
        text: "Escribir un indicativo, Enter — el mapa vuela hasta donde QRZ sitúa la estación, traza el círculo máximo desde la propia ubicación e indica distancia y rumbo. Debajo, imágenes de satélite de NASA GIBS, sin clave. Los satélites de radioaficionado por encima del horizonte aparecen como triángulos en el globo y el mapa, y cada QSO registrado anota cuáles estaban a la vista en ese momento.",
        marken: ["Círculo máximo", "NASA GIBS", "Satélites"] },
      ...
      { bereich: "Equipos ajenos", titel: "Incluso lo que no habla OpenHPSDR",
        ...
        marken: ["SunSDR2 QRP", "KiwiSDR"] },
      { bereich: "Digital", titel: "RTTY directamente del audio de recepción",
      ...
      { bereich: "Digital", titel: "Morse directamente del audio de recepción", neu: true,
        text: "Un decodificador de CW propio, también sin fldigi. Detecta solo el tono y la velocidad, y ambos se pueden fijar; la banda de búsqueda sigue el pitch de CW configurado. Los caracteres dudosos aparecen atenuados, el ruido ni se muestra.",
        marken: ["CWL / CWU", "LOCK Hz / WPM"] },
      { bereich: "Memorias", titel: "Frecuencias y recepción bajo un nombre", neu: true,
        text: "Memorias de frecuencia en el formato de Thetis — un memory.xml existente se puede copiar tal cual — con grupo, nombre, modo, filtro, AGC y desplazamiento de repetidor por posición. Aparte, perfiles RX: AGC, reducción de ruido, notch, supresor de ruido y squelch de un receptor, guardados y recuperados con un clic.",
        marken: ["memory.xml", "Quick Save", "Perfiles RX"] },
    ],
    ...
    download: {
      ...
      standUngeprueft: "Firmado · sin verificar",
      standWindows: "Firmado · sin verificar · problema de audio abierto",
