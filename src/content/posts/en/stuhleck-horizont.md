---
titel: "What the Stuhleck sees"
datum: 2026-09-21
vorspann: "The highest peak of the Fischbacher Alpen has a road up to the hut and a car park on the summit. 360 bearings, 800 kilometres, 3,215 contest logs, plus the laser scan for the hut next door — and a horizon none of the five sites before it has. Only Germany lies half behind the Rax."
aufmacher: "../../../assets/karten/stuhleck-horizont-aufmacher.png"
aufmacherAlt: "Dark map 700 kilometres around the Stuhleck with national borders: the rim arc is green without a break from 11° through east and south to 226°, with short amber pieces to the north-west and north; plus the numbers 11° to 226° and +0.1° to +0.8°"
aufmacherRef: "1,782 m · JN77VN"
aufmacherFormat: "breit"
schlagworte: ["2026", "Fischbacher Alpen", "Contest", "Site", "Antennas"]
---
...
![Site plan of the Stuhleck summit from the laser scan: car park, Alois-Günther-Haus, summit cross, two masts thirty metres west of the car park on the dome with the quad wedges at 20° and 216° and a ring around mast 2 showing the Yagi's loss per degree — green all round, amber only from 96° to 119°](../../../assets/karten/stuhleck-lageplan.png)
...
<figure class="szene">
  <iframe src="/stuhleck-3d.html?v=13" title="Stuhleck summit in 3D: orthophoto on the laser-scan terrain with the Yagi stack on rotator 1, the quad stack on rotator 2, the car, the Steinbachalmbahn top station and the no-go sector towards the chairlift" loading="lazy" allowfullscreen></iframe>
  <figcaption>Drag to turn, scroll to zoom. The buttons set the fixed directions — Yagi stack 302° / 18° / 86°, quad stack 20° / 116° / 120° / 136° / 200° —, the sliders any other; below, per stack, the stations, countries and cities in the main beam and a warning when a direction lies in the no-go sector. “Drehen” lets the view circle slowly, “Stopp” holds it; “Bergstation” swings to the chairlift. <a href="/stuhleck-3d.html">Open full screen</a></figcaption>
</figure>
...
![Comparison sheet with two dark station maps, 700 kilometres around the Stuhleck: site A on the dome at left, site B on the meadow at right, in both a red no-go sector towards the chairlift and a paler one turned 180 degrees, plus the quad wedges and the three Yagi positions; below, the numbers: without the no-go 1,473 against 1,331, with it 1,342 against 1,325 stations under three decibels](../../../assets/karten/stuhleck-standortvergleich.png)
...
The result turns the first plan around. **The two 12JXX2 go stacked on mast 2, rest position 302°** — Regensburg, Nuremberg, Passau, Bayreuth, Hof, at 17.8 dBi, Munich at the beam’s edge with thirteen, which is enough for two hundred kilometres. **The two quads go stacked on the rotator of mast 1**, 69 degrees wide, 14.5 dBi, three positions: **20°** Wrocław, Brno, Ostrava, Kraków, Vienna — the second big block, 405 stations; **108°** Budapest, Belgrade, Hungary, Serbia; **200°** Zagreb, Rijeka, Ljubljana, Trieste, Venice, as far as the no-go allows. The near range lies in two broad blocks that a wide antenna serves without constant turning; the far range needs the gain only the stack has — and Germany needs both.

And the Yagi stack gets a rotator of its own. The rest position stays 302°, but two more positions fetch what no antenna reached with gain before: **330°** Prague, Dresden, Leipzig, Berlin — Germany’s north-east behind the Rax — and **266°** Innsbruck, Zurich, Bolzano, hard against the no-go edge. That is the biggest single gain of the whole calculation: 994 stations within 500 kilometres and 1,351 within 750, against 968 and 1,205 with the stack fixed and 872 and 987 for the first plan. In Germany 419 of the 1,070, 130 of them small — the first plan had 159 and 49, because a quad with twelve dBi no longer reaches a small station beyond four hundred kilometres.

The quads separated into two directions, back to back on the rotator, were the other idea — dropped, and the power says why. With a thousand watts going to all antennas at once, each system gets its share: two systems at 500 watts, three at 333. Two quads in two directions then have twelve dBi each with 333 watts, the quad stack 14.5 with 500 — 4.3 decibels per direction, and the counting rule turns that into 520 instead of 871 stations within 750 kilometres. And both are weaker than the installation that transmits on one system at a time and listens on the others: a thousand watts into whichever stack is on gives the 1,351. Transmit on one antenna, listen on all — that is the rule that follows from the numbers. Whoever nonetheless always transmits on all and only switches the listening gets different best positions with 500 watts per stack: the Yagi stack stays at rest on 302° but goes to **18°** (Kraków, Žilina) and **86°** (Košice, Budapest) instead of Saxony and Switzerland, because the far range behind the Rax is out of reach at half power anyway; the quads on 20°, 136°, 200°. That gives 854 and 950 stations, Germany 226, 64 of them small — a third less than with the switch, in Germany almost half. If the Yagi stack always stays on 302° and only the quad stack turns (20°, 116°, 200°), it is 783 and 817 — Germany the same, the seventy missing are Kraków and Košice with the Yagi gain. An unequal divider, two thirds for the Yagi stack, would give 1,010 within 750 kilometres and 262 German.

The masts are shorter than thought: ten metres for the quads, seven and a half for the Yagis. On the dome that changes nothing — the laser scan recalculated for every height gives the same numbers at eight metres as at thirteen, because in every direction only the hut stands in the near field, and no position points there for long. What counts is the stack, not the height: a mast with only one antenna costs a hundred stations in the near range. Mast 1 on the dome, twelve metres from the summit cross, carries the quads at 6.4 and 9.7 metres; mast 2, twelve metres further north-west, the Yagis at 4.0 and 7.5 — from the quad mast the Yagi mast lies at 328°, outside every quad position, and from the Yagi mast the quads lie at 148°, behind the stack at rest. The Yagi mast is guyed at 3.3 metres, below rotator and lower Yagi, so that no wire runs through the elements.
...
| 1,000 W on all at once | two systems at 500 W, positions re-optimised: Yagi 302° / 18° / 86°, quads 20° / 136° / 200° → 854 / 950, Germany 226 / 64 · 2:1 divider for the Yagi stack: 830 / 1,010, Germany 262 / 80 · three systems at 333 W with quads in two directions: 520 / 520 — hence no back to back |
